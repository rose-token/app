...
```bash
# Start webhook server (serves webhooks + frontend if built)
npm start
...
**Purpose**: The webhook server handles Alchemy webhook events to automatically merge GitHub pull requests when tasks are approved on-chain.

**Key Features**:
- Listens for `TaskApproved` events from RoseMarketplace contract
- Decodes event data to extract GitHub PR URL, task ID, and worker address
- Automatically merges the PR via GitHub API when task is approved
- Serves the built React frontend as static files (optional)
...
2. **Environment Variables**:
   ...
   MARKETPLACE_ADDRESS=0x...  # Optional, auto-loads from deployment-output.json
   ```
3. **Alchemy Webhook**: Configure Custom Webhook with event topic filter (recommended for development)

**Event Filtering**:
- **By Event Signature**: Filters for `TaskApproved(uint256,address,string)` event only
  - Topic 0: `0x09aa7df800a4b4e4068aa4599c1246b9518840f47a2ff262f1273c49bf117d82`
  - Server displays this signature on startup
- **By Contract Address**: Automatically reads from `deployment-output.json`
  - Set `MARKETPLACE_ADDRESS` env var to override
  - Warns if not configured (accepts all contracts in development)

**Workflow**:
1. Deploy contracts → `deployment-output.json` created with addresses
2. Start webhook server → auto-loads marketplace address
3. Configure Alchemy webhook with event topic filter (use signature from server logs)
4. Approve task on-chain → Alchemy sends webhook → Server merges PR

**Handling Redeployments**:
- **Recommended (Custom Webhook with Topic Filter)**:
  - Configure Alchemy to filter by event signature only (no contract address)
  - Server verifies contract address from `deployment-output.json`
  - No Alchemy reconfiguration needed after redeploy - just restart server
- **Alternative (Address Activity Webhook)**:
  - Must update Alchemy webhook with new contract address after each deploy
  - More secure but requires manual updates

**Frontend Serving**:
- Priority 1: Serves `frontend/build/` (production mode) if it exists
- Priority 2: Serves `frontend/public/` (development mode) as fallback
- Exits with error if neither directory exists
- Production build: `cd frontend && npm run build`

**Endpoints**:
- `GET /health` - Health check for monitoring
- `POST /webhook/task-approved` - Alchemy webhook receiver
- `GET /*` - Serves frontend (build or public directory)

See `WEBHOOK_SERVER.md` for detailed documentation, deployment instructions, and troubleshooting.
